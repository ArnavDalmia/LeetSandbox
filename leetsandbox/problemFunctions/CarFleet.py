...
Number of car fleets: {result}"""
    ...
    for i, (pos, spd) in enumerate(pairs):
        ...
        output += f"\n  Car {i+1}: Position {pos}, Speed {spd}, Time to reach target: {time_to_target:.2f}"
    ...
    output += f"\n  Fleet 1: Car at position {pairs[0][0]} (time: {fleet_time:.2f})"
    ...
    for i in range(1, len(pairs)):
        ...
        if time_to_target > fleet_time:
            ...
            output += f"\n  Fleet {fleets}: Car at position {pos} (time: {time_to_target:.2f}) - forms new fleet"
        else:
            output += f"\n  Car at position {pos} (time: {time_to_target:.2f}) - joins previous fleet"
